...
namespace TASK_18
{
    class Program
    {
        ...
        {
            string x = "";//для счета строк в файле (не нашла способа лучше)
            int line_count=0;
            ...
            using (StreamReader sr = new StreamReader($"res_{path}", System.Text.Encoding.Default))
            {
            ...
            for (int A = 0; A < line_count-1; ++A)
            {
                ...
                /* int c = 0;
                 ...
                 {
                     if (!final_state.Contains(all_states[i]))
                     { 
                         foreach(string s in content[i])
                             if(!s.StartsWith("f_"))
                ...
                for (int i = 0; i < content.Count; ++i)
                {
                    ...
                    {
                        if (final_state.Contains(content[i][j]) && content[i][j] != all_states[i] && !all_states[i].StartsWith("f_"))
                        {
                            for (int k = 0; k < content.Count; ++k)
                                for (int l = 0; l < content[k].Count; ++l)
                                {
                                    if (content[k][l] == all_states[i])
                                    {
                                        content[k][l] = "f_" + content[k][l];
                                    }
                                }
                            all_states[i] = "f_" + all_states[i];
                            final_state.Add(all_states[i]);
                        }
                    }
                }
